# Edgar Daniel Díaz Nava

Mathematically-driven Data Scientist - Machine Learning Engineer  
Reinforcement Learning · Optimization · Quantitative Modeling · Simulation Systems  

---

## Research & Engineering Focus

- Stochastic Processes & Probabilistic Modeling
- Reinforcement Learning (Single & Multi-Agent)
- Optimization & Control Systems
- Geospatial Network Modeling
- Simulation-Based Decision Systems
- Quantitative Finance Research (Portfolio Optimization, Risk Modeling)
- Data Visualization and strategic Insight 

---

## Core Stack

### Languages
[![Core](https://skillicons.dev/icons?i=python,r,sql,bash,linux,git,pyspark)](https://skillicons.dev)

### Machine Learning
[![ML](https://skillicons.dev/icons?i=pytorch,tensorflow,sklearn)](https://skillicons.dev)

### Quantitative & Scientific Computing
![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy)
![SciPy](https://img.shields.io/badge/SciPy-8CAAE6?style=for-the-badge&logo=scipy)
![Statsmodels](https://img.shields.io/badge/Statsmodels-003366?style=for-the-badge)
![CVXPY](https://img.shields.io/badge/CVXPY-004F9F?style=for-the-badge)
![PyMC](https://img.shields.io/badge/PyMC-6A1B9A?style=for-the-badge)

### Reinforcement Learning & Simulation
![Stable Baselines3](https://img.shields.io/badge/Stable%20Baselines3-FF6F00?style=for-the-badge)
![SUMO](https://img.shields.io/badge/SUMO-Traffic_Simulation-red?style=for-the-badge)
![Ray RLlib](https://img.shields.io/badge/Ray%20RLlib-028CF0?style=for-the-badge)

### Data Engineering & Systems
[![Infra](https://skillicons.dev/icons?i=docker,aws,postgres)](https://skillicons.dev)

---

## Selected Technical Projects

### Multi-Agent Reinforcement Learning for Urban Traffic Control
- Custom MARL architecture over SUMO
- Network-level geospatial adjacency extraction
- Reward shaping under congestion constraints
- Parallel environment simulation
- GPU acceleration with PyTorch

### Bayesian & GLM Modeling Research
- Negative Binomial Dynamic GLMs
- Overdispersion modeling in count processes
- Hierarchical Bayesian estimation

### Sports Event Probabilistic Modeling
- Dixon-Coles extensions
- Market-implied probability extraction
- Calibration diagnostics

### Portfolio Optimization & Capital Allocation
- Convex optimization via CVXPY
- Efficient frontier simulation
- Regime-based allocation strategies


---

## Mathematical Foundation

- Measure-theoretic probability
- Convex optimization
- Statistical inference (frequentist & Bayesian)
- Stochastic processes
- Control theory foundations

---

## Current Research Directions

- Multi-Agent RL in High-Stress Network Systems
- Simulation-to-Real Transfer in Decision Systems
- Dynamic Risk Allocation under Uncertainty
- Optimization-driven Policy Learning

---

## Contact

LinkedIn: [https://www.linkedin.com/in/edgardaniel/]  
Email: eddn.professional@proton.me 



---

## GitHub Metrics

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=edgar-daniel-codes&show_icons=true&hide_border=true&theme=default)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=edgar-daniel-codes&layout=compact&hide_border=true)

